...
const NETWORK_WARNING_THROTTLE_MS = 15_000;

function warnNetworkUnavailable(attemptedUrl: string) {
  const now = Date.now();
  if (now - lastNetworkWarning < NETWORK_WARNING_THROTTLE_MS) return;

  lastNetworkWarning = now;
  console.warn(
    `[apiClient] Backend temporarily unreachable: ${attemptedUrl || API_URL}`
  );
}
...
function isExpectedMailTransientResponse(
  status: number | undefined,
  requestUrl: string
): boolean {
  // 304 used to be generated by Express conditional GET handling for dynamic
  // /api/mail JSON responses. Stage 1.4.1 prevents it server-side, but keep
  // this warning-only fallback so an older backend cannot open the Next.js
  // development overlay while rolling deployments are crossing versions.
  // 504 is the new deliberate Gmail-provider hard deadline: callers already
  // catch it and preserve/reconcile their current UI state.
  return (
    requestUrl.includes("/api/mail/") &&
    (status === 304 || status === 504)
  );
}

function isExpectedMailClientTimeout(
  error: any,
  requestUrl: string
): boolean {
  if (!requestUrl.includes("/api/mail/")) return false;
    ...
    this.client.interceptors.response.use(
      ...
      async (error) => {
        ...
        if (error.code === "ERR_NETWORK" || error.message === "Network Error") {
          ...
          // Keep rejecting the request exactly as before, but avoid turning one
          // temporary backend outage into dozens of Next.js development overlays.
          // A single throttled warning is enough to preserve diagnostics.
          warnNetworkUnavailable(attemptedUrl);
        } else if (isExpectedMailClientTimeout(error, requestUrl)) {
          // The backend now has shorter provider deadlines, so this should be a
          // last-resort client safety net only. The caller still receives the
          // rejection and keeps its existing toast/reconciliation behavior.
          console.warn(
          ...
          if (isExpectedMailTransientResponse(status, requestUrl)) {
            console.warn(
              `[apiClient] Suprah One Desk mail request temporarily unavailable (${status}):`,
              error.response.data
            );
          } else if (isExpectedSupraSpaceAvailabilityError(status, requestUrl)) {
