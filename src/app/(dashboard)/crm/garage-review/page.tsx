...
export default function GarageReviewPage() {
  ...
  const refreshHistory = React.useCallback(async (t: string) => {
    try {
      setTransfers(await fetchTransfers(t));
    } catch {
      /* non-fatal */
    }
  }, []);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      // reset selection, keep history fresh
      ...
      <div className="w-full px-4 sm:px-6 py-8 space-y-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 dark:text-zinc-500 mt-0.5">
              Transfer a sold vehicle into the customer&apos;s My Garage at deal close
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <Card className="p-5 rounded-2xl border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40">
            <div className="flex items-center justify-between mb-3">
              ...
              {selectedVehicle && (
                <button
                  ...
                  className="text-[11px] text-zinc-400 hover:text-red-500 flex items-center gap-1"
                >
            ...
            {selectedVehicle ? (
              <SelectedRow
                ...
                sub={`VIN ${selectedVehicle.vin}`}
                meta={selectedVehicle.status}
              />
            ) : (
              <>
                ...
                <ResultList loading={loadingVehicles} empty="No matching inventory.">
                  {vehicles.map((v) => (
                    <button
                      ...
                      className="w-full text-left flex items-center gap-3 p-2.5 rounded-xl border border-transparent hover:border-emerald-400/50 hover:bg-emerald-50/40 dark:hover:bg-emerald-950/20 transition-colors"
                    >
                      ...
                      <Badge className={cn("text-[9px] h-5 px-2 rounded-full border-0 shrink-0", statusTone(v.status))}>
                        {v.status}
                      </Badge>
                    </button>
                  ))}
          ...
          <Card className="p-5 rounded-2xl border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40">
            ...
            {selectedCustomer ? (
              <SelectedRow
                image={resolveImageUrl(selectedCustomer.avatar || undefined) || null}
                title={selectedCustomer.name}
                sub={selectedCustomer.email}
                meta={selectedCustomer.role}
              />
            ) : (
              <>
                <div className="relative mb-3">
                ...
                <ResultList loading={loadingCustomers} empty="No matching customers.">
                  {customers.map((c) => (
                    ...
                    >
                      <Thumb
                        image={resolveImageUrl(c.avatar || undefined) || null}
                        fallback={<UserIcon className="h-4 w-4 text-zinc-400" />}
                      />
                      <div className="flex-1 min-w-0">
                      ...
                      <Badge className="text-[9px] h-5 px-2 rounded-full capitalize bg-zinc-100 dark:bg-zinc-800 text-zinc-500 border-0 shrink-0">
                        {c.role}
                      </Badge>
                    </button>
                  ))}
        ...
        <Card className="p-5 rounded-2xl border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <select
                ...
                className="w-full h-9 rounded-md border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/30"
              >
                <option value="">No deal — standalone transfer</option>
                {deals.map((d) => (
                  <option key={d._id} value={d._id}>
                    {d.title}
                    {d.contactName ? ` · ${d.contactName}` : ""} ({d.stage.replace("_", " ")})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-4 border-t border-zinc-100 dark:border-zinc-800">
            <p className="text-xs text-zinc-400">
              {selectedVehicle && selectedCustomer ? (
                <>
                  ...
                  . The vehicle will be marked <strong>Sold</strong> and appear in their garage instantly.
                </>
              ) : (
            ...
            <Button
              ...
              className="rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold gap-2 h-11 px-6 shrink-0 disabled:opacity-40"
            >
          ...
          {transfers.length === 0 ? (
            ...
            <div className="space-y-2">
              {transfers.map((t) => (
                <div
                  key={t._id}
                  className="flex items-center gap-3 p-3.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-zinc-900 dark:text-white truncate">
                      {t.vehicleLabel} → {t.customerName}
                    </p>
                    <p className="text-[11px] text-zinc-400 truncate">
                      {t.customerEmail} · {t.mileageAtTransfer.toLocaleString()} mi · by {t.performedByName}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    {t.status === "already_in_garage" ? (
                      <Badge className="text-[9px] h-5 px-2 rounded-full bg-amber-100 text-amber-600 border-0">
                        Refreshed
                      </Badge>
                    ) : (
                      <Badge className="text-[9px] h-5 px-2 rounded-full bg-emerald-100 text-emerald-600 border-0">
                        Transferred
                      </Badge>
                    )}
...
function Thumb({ image, fallback }: { image: string | null; fallback: React.ReactNode }) {
  return (
    <div className="h-10 w-10 rounded-lg overflow-hidden bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center shrink-0">
      {image ? (
        <img src={resolveImageUrl(image) || image} alt="" className="h-full w-full object-cover" />
      ) : (
        fallback
      )}
    </div>
...
function SelectedRow({
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl border border-emerald-300 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/20">
      ...
      <Badge className="text-[9px] h-5 px-2 rounded-full capitalize bg-white/70 dark:bg-zinc-900/70 text-zinc-500 border-0 shrink-0">
        {meta}
      </Badge>
    </div>
  );
