...
export function TeamCalendar({
  ...
  const [pendingProofId, setPendingProofId] = React.useState<string | null>(null);
  ...
  const myDayAbsence = selectedDay
    ? absences.filter((a) => isSameDay(parseISO(a.date), selectedDay)).find((a) => a.userName === userName)
    : undefined;
  ...
  async function handleReject(id: string) {
    try {
      await rejectAbsence.mutateAsync({ id, reason: rejectReason.trim() || undefined });
  ...
  function ProofAttachments({ attachments }: { attachments: NonNullable<Absence["proofAttachments"]> }) {
        ...
        {attachments.map((att, i) => {
          ...
          return isImage ? (
            <button
              key={i}
              type="button"
              onClick={() => setLightbox({ url: att.url, name: att.originalName })}
    ...
    {lightbox && (
      <Dialog open onOpenChange={(o) => { if (!o) setLightbox(null); }}>
        <DialogContent className="max-w-4xl w-[95vw] max-h-[92vh] p-0 bg-black/90 border-0 overflow-hidden flex items-center justify-center rounded-2xl">
          ...
          <img src={lightbox.url} alt={lightbox.name} className="max-w-full max-h-[90vh] object-contain rounded-lg" />
          <p className="absolute bottom-3 left-1/2 -translate-x-1/2 text-white/50 text-xs truncate max-w-xs">{lightbox.name}</p>
        </DialogContent>
      </Dialog>
    )}
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
        <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-border/30">
          {[
            { label: "Out This Month", value: monthStats.total, sub: `${monthStats.unique} people`, accent: "text-rose-600 dark:text-rose-400" },
            { label: "Out Today", value: todayAbsences.length, sub: todayAbsences.length ? "on leave / absent" : "everyone in", accent: todayAbsences.length ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground/60" },
            { label: "This Week", value: upcomingAbsences.length, sub: "upcoming absences", accent: "text-violet-600 dark:text-violet-400" },
            { label: "My Schedule", value: myMonthAbsences.length, sub: `entries in ${format(calMonth, "MMM")}`, accent: "text-emerald-600 dark:text-emerald-400" },
          ].map((s) => (
            <div key={s.label} className="flex flex-col gap-1 px-4 sm:px-5 py-4">
              <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 leading-none">{s.label}</p>
              <p className={cn("text-3xl sm:text-4xl font-black tabular-nums leading-none mt-1", s.accent)}>{s.value}</p>
              <p className="text-[10px] text-muted-foreground/50 leading-tight mt-0.5">{s.sub}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        <div className="lg:col-span-7 rounded-xl border border-border/50 bg-card overflow-hidden">
          <div className="flex items-center justify-between gap-3 px-4 py-3 border-b border-border/30">
            <div>
              <h2 className="font-black text-base tracking-tight leading-none">{format(calMonth, "MMMM yyyy")}</h2>
              <p className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground/50 mt-0.5">Team Calendar · Click a date</p>
            </div>
            <div className="flex items-center gap-0.5 border border-border/40 rounded-lg p-0.5 bg-background shrink-0">
              <Button variant="ghost" size="icon" className="size-7 rounded-md" onClick={() => setCalMonth(new Date(calYear, calMonthNum - 2, 1))}>
                <ChevronLeft className="size-3.5" />
              </Button>
              <Button variant="ghost" size="sm" className="h-7 px-2 rounded-md text-[11px] font-black" onClick={() => setCalMonth(new Date())}>Today</Button>
              <Button variant="ghost" size="icon" className="size-7 rounded-md" onClick={() => setCalMonth(new Date(calYear, calMonthNum, 1))}>
          ...
          <div className="flex flex-wrap gap-2 px-4 py-2.5 border-b border-border/30 bg-muted/20">
              ...
              {(calDeptFilter !== "all" || calTypeFilter !== "all") && (
                <Button variant="ghost" size="sm" className="h-7 text-xs px-2 gap-1 text-muted-foreground" onClick={() => { setCalDeptFilter("all"); setCalTypeFilter("all"); }}>
            ...
            <div className="mt-4 pt-3 border-t border-border/20">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1.5">
                {(Object.entries(A) as [AbsenceType, { label: string; dot: string; pill: string }][]).map(([t, c]) => (
                  <button
                    key={t}
                    onClick={() => setCalTypeFilter(calTypeFilter === t ? "all" : t)}
          ...
          {upcomingAbsences.length > 0 && (
            <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/30">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">Next 7 Days</span>
                <span className="ml-auto text-[10px] font-black text-violet-600 dark:text-violet-400 tabular-nums">{upcomingAbsences.length}</span>
              </div>
              <div className="overflow-y-auto px-3 py-2.5 space-y-1.5" style={{ maxHeight: 192, scrollbarWidth: "none" }}>
                {upcomingAbsences.map((a) => {
                  const liveAvatar = members.find((m) => m.name === a.userName)?.avatar;
                  const cfg = A[a.type as AbsenceType];
                  return (
                    <div key={a._id} className="flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg hover:bg-muted/20 transition-colors">
                      ...
                      <span className={cn("text-[9px] font-black px-1.5 py-0.5 rounded shrink-0", cfg.pill)}>{cfg.label}</span>
                      {a.status === 'pending' && (
                        <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 dark:bg-amber-950/50 dark:text-amber-400 shrink-0">pending</span>
                      )}
                    </div>
                  );
                })}
          ...
          {selectedDay ? (
            <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
              <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-border/30">
                <div>
                  <h3 className="font-black text-base tracking-tight leading-none">{format(selectedDay, "EEEE")}</h3>
                  <p className="text-sm text-muted-foreground/70 font-semibold mt-0.5">{format(selectedDay, "MMMM d, yyyy")}</p>
                ...
                <Button variant="ghost" size="icon" className="size-7 text-muted-foreground/40 hover:text-foreground shrink-0 mt-0.5" onClick={() => setSelectedDay(null)}>
              ...
              <div className="p-4 space-y-3">
                {dayAbsences.length > 0 && (
                  <div className="overflow-y-auto space-y-1.5" style={{ maxHeight: 240, scrollbarWidth: "none" }}>
                    {dayAbsences.map((a) => {
                      const cfg = A[a.type as AbsenceType];
                  ...
                  <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-muted/30 border border-border/20">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground/60">
                        ...
                        {myDayAbsence.status === 'pending' && " · pending approval"}
                      </p>
                    </div>
                  ...
                  <div className="rounded-xl border border-border/30 bg-muted/20 p-3 space-y-2.5">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 space-y-0.5">
                        <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">End Date</label>
                        <Input
                          type="date"
                          value={absEndDate}
                      ...
                      {absEndDate && (
                        <button className="mt-4 text-muted-foreground/40 hover:text-foreground" onClick={() => setAbsEndDate("")}>
                          <X className="size-3.5" />
                        </button>
                      )}
                    </div>
                    <p className="text-[9px] text-muted-foreground/40">Leave End Date empty for a single day</p>
          ...
          {myMonthAbsences.length > 0 && (
            <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/30">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50">My Schedule · {format(calMonth, "MMM yyyy")}</span>
                <span className="ml-auto text-[10px] font-black text-emerald-600 dark:text-emerald-400 tabular-nums">{myMonthAbsences.length}</span>
              </div>
              <div className="overflow-y-auto px-3 py-2 space-y-0.5" style={{ maxHeight: 200, scrollbarWidth: "none" }}>
                {myMonthAbsences.map((a) => {
                  ...
                  return (
                    <div key={a._id} className="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-muted/30 group/e transition-colors">
                      ...
                      <Button variant="ghost" size="icon" className="size-5 opacity-100 sm:opacity-0 sm:group-hover/e:opacity-100 text-muted-foreground hover:text-destructive transition-all shrink-0" onClick={() => handleDeleteAbsence(a._id)}>
                        <Trash2 className="size-3" />
                      </Button>
                    </div>
                  );
