...
type TabId = "overview" | "team" | "activity" | "calendar" | "board";
...
export default function TeamPulsePage() {
  ...
  const now = new Date();
  const { data: absences = [] } = useTeamAbsences(now.getFullYear(), now.getMonth() + 1);
  ...
  async function applyStatus() {
    ...
    try {
      ...
      const label = onlineStatusOptions.find((o) => o.value === draftStatus)?.label ?? draftStatus;
      toast.success(`Status set to ${label}${draftCustom.trim() ? ` · "${draftCustom.trim()}"` : ""}`);
    } catch {
      setMyStatus(prev);
  ...
  const currentStatusOpt = onlineStatusOptions.find((o) => o.value === myStatus) ?? onlineStatusOptions[0];
  ...
  const tabs: { id: TabId; Icon: React.ComponentType<{ className?: string }>; label: string; count?: number }[] = [
    ...
    { id: "activity",  Icon: Activity,     label: "Activity", count: activeCount > 0 ? activeCount : undefined },
            ...
            <div className="flex items-center justify-between gap-4 py-4 sm:py-5">
              <div className="flex items-center gap-3 min-w-0">
                ...
                {members.length > 0 && (
                  <div className="hidden sm:flex items-center gap-1.5 pl-3 border-l border-border/40">
                    <span className="text-[11px] font-black tabular-nums text-foreground">{members.length}</span>
                    <span className="text-[11px] text-muted-foreground/60">members</span>
                    <span className="text-[11px] text-muted-foreground/30">·</span>
                    <span className="size-1.5 rounded-full bg-green-500 shrink-0" />
                    <span className="text-[11px] font-black tabular-nums text-green-700 dark:text-green-400">{onlineCount}</span>
                    <span className="text-[11px] text-muted-foreground/60">online</span>
                    {todayAbsences.length > 0 && (
                      <>
                        <span className="text-[11px] text-muted-foreground/30">·</span>
                        <span className="text-[11px] font-black tabular-nums text-amber-700 dark:text-amber-400">{todayAbsences.length}</span>
                        <span className="text-[11px] text-muted-foreground/60">out</span>
                      </>
                    )}
                  </div>
                )}
              ...
              <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className={cn("size-2 rounded-full shrink-0", currentStatusOpt.color)} />
                    ...
                    {expiryLabel && (
                      <span className="hidden md:flex items-center gap-0.5 text-muted-foreground/50 text-[9px]">
                        <Clock className="size-2.5" />{expiryLabel}
                      </span>
                    )}
                ...
                <PopoverContent align="end" className="w-64 p-0 rounded-xl overflow-hidden">
                  <div className="px-3 pt-3 pb-1">
                    <p className="text-[9px] font-black text-muted-foreground/50 uppercase tracking-widest">My Status</p>
                  </div>
                  ...
                  <div className="px-1.5 space-y-0.5 pb-1">
                    {onlineStatusOptions.map((opt) => (
                      <button
                        key={opt.value}
                        onClick={() => setDraftStatus(opt.value as OnlineStatus)}
                        className={cn(
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-muted-foreground/60 truncate">{opt.description}</p>
                        </div>
                        {draftStatus === opt.value && <Check className="size-3.5 text-primary shrink-0" />}
                      </button>
                    ))}
                  </div>

                  <div className="border-t border-border/40 px-3 pt-2.5 pb-1.5 space-y-2">
                    <p className="text-[9px] font-black text-muted-foreground/50 uppercase tracking-widest">Custom Status</p>
                    <div className="relative">
                      <Input
                        value={draftCustom}
                        onChange={(e) => setDraftCustom(e.target.value.slice(0, 60))}
                  ...
                  <div className="px-3 pb-2.5 space-y-1.5">
                    <p className="text-[9px] font-black text-muted-foreground/50 uppercase tracking-widest">Clears In</p>
          ...
          <div className="animate-in fade-in duration-200">
            {tab === "overview" && (
              <div className="space-y-6">
                <TeamOverview members={members} todayAbsences={todayAbsences} onMemberClick={(id) => setOpenMemberId(id)} myUserId={myUserId} />
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40 mb-3">Quick Access</p>
                    ...
                    {[
                      {
                        ...
                        borderCls: "border-border/40 hover:border-blue-300/50 dark:hover:border-blue-700/40",
                      ...
                      },
                      {
                        ...
                        borderCls: "border-border/40 hover:border-green-300/50 dark:hover:border-green-700/40",
                      ...
                      },
                      {
                        ...
                        Icon: CalendarDays,
                        label: "Team Calendar",
                        desc: todayAbsences.length ? `${todayAbsences.length} out today` : "All in today",
                        accentCls: "bg-amber-500",
                        borderCls: "border-border/40 hover:border-amber-300/50 dark:hover:border-amber-700/40",
                      ...
                      },
                      {
                        ...
                        borderCls: "border-border/40 hover:border-emerald-300/50 dark:hover:border-emerald-700/40",
                        iconCls: "text-emerald-600 dark:text-emerald-400",
                      },
                    ].map(({ id, Icon, label, desc, accentCls, borderCls, iconCls }) => (
                      <button
                        key={id}
                        onClick={() => setTab(id)}
                        className={cn(
                      ...
                      >
                        <div className={cn("absolute top-0 left-0 w-0.5 h-full opacity-60 group-hover:opacity-100 transition-opacity", accentCls)} />
                        <Icon className={cn("size-4 shrink-0 transition-transform group-hover:scale-110", iconCls)} />
                        <div className="min-w-0 w-full">
                          <p className="text-[11px] sm:text-xs font-bold truncate">{label}</p>
                          <p className="text-[9px] sm:text-[10px] text-muted-foreground/60 truncate mt-0.5">{desc}</p>
                        </div>
                      </button>
                    ))}
                  </div>
            ...
            {tab === "calendar" && (
              <TeamCalendar members={members} userName={user?.fullName} isAdmin={isAdmin} myUserId={myUserId} />
            )}

            {tab === "board" && (
              <TeamBoard userName={user?.fullName} myUserId={myUserId} isAdmin={isAdmin} />
            )}
          </div>
        </div>

        <MemberProfileSheet memberId={openMemberId} onClose={() => setOpenMemberId(null)} myUserId={myUserId} />
      </div>
    </TooltipProvider>
  );
}
