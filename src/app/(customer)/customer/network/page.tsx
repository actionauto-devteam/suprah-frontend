    ...
    <div className="flex flex-col pb-4 animate-in fade-in slide-in-from-bottom-4 duration-700 lg:h-[calc(100vh-8rem)] lg:pb-0">
      ...
      <div className="flex lg:hidden gap-1.5 p-1 bg-zinc-100 dark:bg-zinc-900 rounded-xl mb-4 shrink-0">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-semibold rounded-lg transition-all touch-manipulation ${mobileTab === 'list'
        ...
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-sm font-semibold rounded-lg transition-all touch-manipulation ${mobileTab === 'map'
            ? 'bg-white dark:bg-zinc-800 shadow text-foreground'
            : 'text-muted-foreground hover:text-foreground'
            }`}
        >
      ...
      <div className="flex flex-col lg:flex-row lg:gap-6 lg:flex-1 lg:min-h-0">
        ...
        {/* Left: Location list */}
        <div className={`${mobileTab === 'list' ? 'block' : 'hidden'} lg:block w-full lg:w-100 lg:flex-col lg:gap-4 lg:overflow-y-auto lg:pr-2 lg:pb-8 custom-scrollbar`}>
          {/* Membership card — always visible above the location list, synced with the full card on the Membership page */}
          <MembershipCardInline onClick={() => setCardOpen(true)} />
          ...
          <div className="flex flex-col gap-3 pb-24 lg:pb-4 shrink-0">
            {isLoading ? (
              ...
              <Card
                ...
                className={`p-4 transition-all cursor-pointer touch-manipulation ${userCoords && i === 0
                  ? 'bg-primary/5 dark:bg-primary/10 border-primary/30 relative overflow-hidden'
              ...
              >
                {userCoords && i === 0 && (
                  <div className="absolute top-0 right-0 bg-primary text-primary-foreground text-[10px] px-1.5 py-0.5 font-bold uppercase tracking-wider rounded-bl-lg">
                    Closest
                  </div>
                )}

                <div className="flex justify-between items-start mb-1 pr-12">
                  <h4 className={`font-semibold text-sm ${userCoords && i === 0 ? 'text-primary' : 'text-foreground'}`}>
                    {loc.name}
                  </h4>
                  {loc.distance && (
