...
const SOURCE_META: Record<PointSourceType, { icon: React.ElementType; tone: string; label: string }> = {
  aftermarket_order:   { icon: ShoppingBag,   tone: "text-emerald-500 bg-emerald-500/10",   label: "Parts Order" },
  service_appointment: { icon: Calendar,      tone: "text-sky-500 bg-sky-500/10",           label: "Service Visit" },
  test_drive:          { icon: Car,           tone: "text-amber-500 bg-amber-500/10",       label: "Test Drive" },
  referral_conversion: { icon: Gift,          tone: "text-fuchsia-500 bg-fuchsia-500/10",   label: "Referral" },
  profile_completion:  { icon: UserCheck,     tone: "text-emerald-500 bg-emerald-500/10",   label: "Profile Bonus" },
  account_anniversary: { icon: CalendarClock, tone: "text-amber-500 bg-amber-500/10",       label: "Anniversary" },
  admin_adjustment:    { icon: ShieldCheck,   tone: "text-zinc-500 bg-zinc-500/10",         label: "Adjustment" },
};
...
function Odometer({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      {digits.map((d, i) =>
        d === "," ? (
          <span key={i} className="px-0.5 text-2xl font-black text-white/40 sm:text-3xl">,</span>
        ) : (
          <span
            key={i}
            className="rounded-md bg-black/35 px-1.5 py-1 font-mono text-2xl font-black tabular-nums text-white shadow-inner ring-1 ring-white/10 sm:text-3xl"
          >
            {d}
          </span>
        ),
...
export default function MembershipPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4 pb-24 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {stats.map((s) => (
          <div key={s.label} className="rounded-2xl border border-border/50 bg-card p-3.5 sm:p-4">
            <div className="flex items-center gap-2">
            ...
            {(allTiers ?? []).map((t) => {
              ...
              return (
                ...
                >
                  ...
                  <p className={cn("text-xs font-black uppercase tracking-tight", isCurrent ? "text-white" : "text-foreground")}>
                    {t.name}
                  </p>
      ...
      <Tabs defaultValue="perks">
                ...
                <table className="w-full min-w-200 text-sm">
                  ...
                  <tbody>
                    {PERK_ROWS.map((row, ri) => (
                      <tr key={row} className="border-b border-border/30 transition-colors last:border-0 hover:bg-muted/20">
        ...
        <TabsContent value="earn">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {EARN_METHODS.map((m) => (
              <div key={m.label} className="group flex items-start gap-3 rounded-2xl border border-border/50 bg-card p-4 transition-colors hover:border-primary/40">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1.5 inline-block rounded-md bg-emerald-500/10 px-2 py-0.5 text-[12px] font-black text-emerald-600 dark:text-emerald-400">
                    {m.pts}
                  </p>
                </div>
              </div>
          ...
          {loadingHistory ? (
            ...
            <div className="flex flex-col gap-2">
              {history.transactions.map((tx) => {
                ...
                return (
                  <div key={tx._id} className="flex items-center gap-3 rounded-2xl border border-border/50 bg-card p-3.5">
                    ...
                    <span className={cn("shrink-0 text-[15px] font-black tabular-nums", tx.delta > 0 ? "text-emerald-500" : "text-red-500")}>
