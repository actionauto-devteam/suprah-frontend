...
import {
  ...
  TrendingUp, Users, XCircle, AlertCircle, Megaphone,
} from "lucide-react";
...
export function AppointmentDashboard() {
  ...
  const { data: statsData } = useQuery({
    ...
    queryFn: async () => {
      const h = await getHeaders();
      const r = await apiClient.get("/api/appointments/dashboard/stats", { ...h, params: { date: selectedDate } });
      return r.data?.data ?? r.data;
    },
    staleTime: 30_000,
  });
        ...
        <div className="flex flex-wrap items-end gap-3 rounded-xl border bg-card px-4 py-3.5 shadow-sm">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-1.5">
              <QuickChip label="Today"    active={selectedDate === today}    onClick={() => handleDateChange(today)} />
              <QuickChip label="Tomorrow" active={selectedDate === tomorrow} onClick={() => handleDateChange(tomorrow)} />
              {selectedDate !== today && <QuickChip label="Reset" active={false} onClick={() => handleDateChange(today)} />}
            </div>
          </div>
