...
import { Loader2, Plus, Pencil, EyeOff, Eye, Building2 } from "lucide-react"
...
export function DepartmentsManager({ token }: DepartmentsManagerProps) {
  ...
  const handleToggleActive = async (dept: DepartmentRow) => {
    ...
    try {
      if (dept.isActive) {
        await apiClient.delete(`/api/crm/departments/${dept._id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        toast.success(`${dept.label} deactivated`)
      } else {
            ...
            <div key={dept._id} className="flex items-center justify-between gap-3 px-4 sm:px-6 py-3">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <Button
                  ...
                  className="h-8 w-8 p-0 rounded-lg"
                >
                  {dept.isActive ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                </Button>
              </div>
            </div>
          ))}
